...
Var *statNative(InterpreterThread &vm, ModuleLoc loc, Span<Var *> args,
...
Var *statIsReg(InterpreterThread &vm, ModuleLoc loc, Span<Var *> args,
	       const StringMap<AssnArgData> &assn_args)
{
	VarStruct *st = as<VarStruct>(args[1]);
...
Var *statIsDir(InterpreterThread &vm, ModuleLoc loc, Span<Var *> args,
	       const StringMap<AssnArgData> &assn_args)
{
	VarStruct *st = as<VarStruct>(args[1]);
	int mode      = as<VarInt>(st->getAttr("mode"))->getVal();
...
Var *statIsChr(InterpreterThread &vm, ModuleLoc loc, Span<Var *> args,
	       const StringMap<AssnArgData> &assn_args)
{
	VarStruct *st = as<VarStruct>(args[1]);
	int mode      = as<VarInt>(st->getAttr("mode"))->getVal();
...
#if !defined(FER_OS_WINDOWS)
Var *statIsBlk(InterpreterThread &vm, ModuleLoc loc, Span<Var *> args,
	       const StringMap<AssnArgData> &assn_args)
{
	VarStruct *st = as<VarStruct>(args[1]);
	int mode      = as<VarInt>(st->getAttr("mode"))->getVal();
...
Var *statIsFifo(InterpreterThread &vm, ModuleLoc loc, Span<Var *> args,
		const StringMap<AssnArgData> &assn_args)
{
	VarStruct *st = as<VarStruct>(args[1]);
	int mode      = as<VarInt>(st->getAttr("mode"))->getVal();
...
Var *statIsLnk(InterpreterThread &vm, ModuleLoc loc, Span<Var *> args,
	       const StringMap<AssnArgData> &assn_args)
{
	VarStruct *st = as<VarStruct>(args[1]);
	int mode      = as<VarInt>(st->getAttr("mode"))->getVal();
...
Var *statIsSock(InterpreterThread &vm, ModuleLoc loc, Span<Var *> args,
		const StringMap<AssnArgData> &assn_args)
{
	VarStruct *st = as<VarStruct>(args[1]);
	int mode      = as<VarInt>(st->getAttr("mode"))->getVal();
