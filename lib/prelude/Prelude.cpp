...
FERAL_FUNC(setMaxRecursion, 1, false, "")
{
    if(!args[1]->is<VarInt>()) {
        vm.fail(loc,
...
INIT_MODULE(Prelude)
{
    ...
    vm.addNativeFn(loc, "getCurrModule", getCurrModule);
    vm.addNativeFn(loc, "getOSName", getOSName);
    vm.addNativeFn(loc, "getOSDistro", getOSDistro);
    ...
    mod->addNativeFn(vm, "setMaxRecursion", setMaxRecursion);
    ...
    mod->addNativeFn(vm, "bytebufferNewNative", bytebufferNewNative);
    // file/filesystem
    mod->addNativeFn(vm, "fsOpen", fsOpen);
    mod->addNativeFn(vm, "fsWalkDir", fsWalkDir);
    mod->addNativeFn(vm, "fsAbsPath", fsAbsPath);
    // file descriptor
    mod->addNativeFn(vm, "fdOpen", fdOpen);
    mod->addNativeFn(vm, "fdRead", fdRead);
    mod->addNativeFn(vm, "fdWrite", fdWrite);
    mod->addNativeFn(vm, "fdClose", fdClose);
    // files and dirs
    ...
    vm.addNativeTypeFn<VarAll>(loc, "_doc_", allGetDoc);
    vm.addNativeTypeFn<VarAll>(loc, "_hasattr_", allHasAttr);
    vm.addNativeTypeFn<VarAll>(loc, "_attr_", allGetAttr);
    ...
    vm.addNativeTypeFn<VarModule>(loc, "_vars_", moduleGetVars);
