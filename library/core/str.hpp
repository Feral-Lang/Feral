...
var_base_t * str_add( vm_state_t & vm, const fn_data_t & fd )
{
	if( fd.args[ 1 ]->type() != VT_STR ) {
		vm.src_stack.back()->src()->fail( fd.idx, "expected string argument for addition, found: %s",
...
var_base_t * str_mul( vm_state_t & vm, const fn_data_t & fd )
{
	if( fd.args[ 1 ]->type() != VT_INT ) {
		vm.src_stack.back()->src()->fail( fd.idx, "expected integer argument for string multiplication, found: %s",
						  vm.type_name( fd.args[ 1 ]->type() ).c_str() );
		return nullptr;
	}
	std::string & lhs = STR( fd.args[ 0 ] )->get();
...
{
	if( fd.args[ 1 ]->type() != VT_STR ) {
		vm.src_stack.back()->src()->fail( fd.idx, "expected string argument for addition assignment, found: %s",
						  vm.type_name( fd.args[ 1 ]->type() ).c_str() );
		return nullptr;
	}
...
{
	if( fd.args[ 1 ]->type() != VT_INT ) {
		vm.src_stack.back()->src()->fail( fd.idx, "expected integer argument for string multiplication assignment, found: %s",
						  vm.type_name( fd.args[ 1 ]->type() ).c_str() );
		return nullptr;
	}
	std::string & lhs = STR( fd.args[ 0 ] )->get();
...
{
	if( fd.args[ 1 ]->type() != VT_STR ) {
		vm.src_stack.back()->src()->fail( fd.idx, "expected string argument for logical less than, found: %s",
						  vm.type_name( fd.args[ 1 ]->type() ).c_str() );
		return nullptr;
	}
	std::string & lhs = STR( fd.args[ 0 ] )->get();
...
{
	if( fd.args[ 1 ]->type() != VT_STR ) {
		vm.src_stack.back()->src()->fail( fd.idx, "expected string argument for logical greater than, found: %s",
						  vm.type_name( fd.args[ 1 ]->type() ).c_str() );
		return nullptr;
	}
	std::string & lhs = STR( fd.args[ 0 ] )->get();
...
{
	if( fd.args[ 1 ]->type() != VT_STR ) {
		vm.src_stack.back()->src()->fail( fd.idx, "expected string argument for logical less than or equal, found: %s",
						  vm.type_name( fd.args[ 1 ]->type() ).c_str() );
		return nullptr;
	}
	std::string & lhs = STR( fd.args[ 0 ] )->get();
...
{
	if( fd.args[ 1 ]->type() != VT_STR ) {
		vm.src_stack.back()->src()->fail( fd.idx, "expected string argument for logical greater than or equal, found: %s",
						  vm.type_name( fd.args[ 1 ]->type() ).c_str() );
		return nullptr;
	}
	std::string & lhs = STR( fd.args[ 0 ] )->get();
