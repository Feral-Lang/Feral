...
bool SimplifyPass::visit(StmtBlock *stmt, Stmt **source)
{
    ...
    for(size_t i = 0; i < stmts.size(); ++i) {
        if(!visit(stmts[i], &stmts[i])) {
            err.fail(stmt->getLoc(), "failed to perform simplify pass on stmt in block");
            return false;
        }
        if(!stmts[i]) {
...
bool SimplifyPass::visit(StmtFnSig *stmt, Stmt **source)
{
    ...
    for(size_t i = 0; i < args.size(); ++i) {
        if(!visit(args[i], asStmt(&args[i]))) {
            err.fail(stmt->getLoc(), "failed to apply simplify pass on function signature arg");
            return false;
        }
        if(!args[i]) {
...
bool SimplifyPass::visit(StmtFnDef *stmt, Stmt **source)
{
    if(!visit(stmt->getSig(), asStmt(&stmt->getSig()))) {
        err.fail(stmt->getLoc(), "failed to apply simplify pass on func signature in definition");
        return false;
    }
    if(!stmt->getSig()) {
        *source = nullptr;
...
bool SimplifyPass::visit(StmtVarDecl *stmt, Stmt **source)
{
    for(size_t i = 0; i < stmt->getDecls().size(); ++i) {
        ...
        if(!visit(d, asStmt(&d))) {
            err.fail(stmt->getLoc(), "failed to apply simplify pass on variable declaration");
            return false;
        }
        if(!d) {
...
bool SimplifyPass::visit(StmtCond *stmt, Stmt **source)
{
    for(auto &c : stmt->getConditionals()) {
        if(c.getCond() && !visit(c.getCond(), &c.getCond())) {
            err.fail(stmt->getLoc(), "failed to apply simplify pass on conditional condition");
            return false;
        }
        if(c.getBlk() && !visit(c.getBlk(), asStmt(&c.getBlk()))) {
            err.fail(stmt->getLoc(), "failed to apply simplify pass on conditional block");
            return false;
        }
    }
    return true;
