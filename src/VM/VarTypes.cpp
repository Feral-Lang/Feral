...
VarError::VarError(ModuleLoc loc, int64_t code, String &&msg)
    : Var(loc), code(code), msg(std::move(msg))
{}
...
VarResult::VarResult(ModuleLoc loc, Var *val, bool isok) : Var(loc), val(val), isok(isok) {}

void VarResult::onCreate(VirtualMachine &vm) { vm.incVarRef(val); }
void VarResult::onDestroy(VirtualMachine &vm) { vm.decVarRef(val); }
